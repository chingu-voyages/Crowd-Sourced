...
function SuccessComp(props) {
	return (
		<div>
			...
			<div className="line" />
			<p>Zip code: {props.data.campaign.location}</p>
			<p>{props.data.campaign.category}</p>
			<p>{props.data.campaign.description}</p>
			<p>{props.data.campaign.itemsNeeded.map((item) => <li>{item}</li>)}</p>
		</div>
	);
}
